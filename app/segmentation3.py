...
def segmentation(logger,modelfiles,basedir=".",models_dir = "nn_models",predictions_dir="predictions",images_dir="images",out_channels=3,writedebug=False):
    ...
    for d in tqdm(listdir(images_dir)):
        print("Predicting nuclei from image"+d)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--user_id',dest="user_id",help="Cytomine User ID")
    parser.add_argument('--ann_id',dest="ann_id",help="Annotation IDs",nargs='+')
    parser.add_argument('--term_id',dest="term_id",help="Term ID to use")
    ...
    with CytomineJob.from_cli(sys.argv) as cj:
            ...
            for annotation in params.ann_id:
                ...
                os.makedirs(params.base_dir+"/"+params.images_dir, exist_ok=True)
                outputfile=params.base_dir+"/"+params.images_dir+"/"+str(annotation)+".png"
                print("Query:",query)
            ...
            coordinates=segmentation(logger,basedir=params.base_dir,modelfiles=params.models,out_channels=params.out_channels,writedebug=params.writedebug,images_dir=params.images_dir,predictions_dir=params.predictions_dir)
            ...
            if params.add_annotations:
                ...
                for p in annotations:
                    ...
                    for point in coordinates[p]:
                        ...
                        json = GenPointJson(annotations[p],point,coordshifts[p],params.term_id,params.user_id)
                        jsonlist.append(json)

                    jarr="["+','.join(jsonlist)+"]"
                    #jarr=''.join(jsonlist)
                    #query = "userannotation.json"
                    query = "algoannotation.json" # Use algoannotation from cytomine gui
